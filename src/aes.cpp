...
void GF8_InitLookup() {
    ...
    for (uint32_t a = 0; a < 256; a++) {
        for (uint32_t b = 0; b < 256; b++) {
            gf8_lookup[a][b] = GF8_Mul(a, b, 0x1bu);
        }
    }
    gf8_lookup_initialized = true;
}

const uint8_t AES_R_CON[11][4] = {
        {0x00, 0x00, 0x00, 0x00},
        {0x01, 0x00, 0x00, 0x00},
        {0x02, 0x00, 0x00, 0x00},
        {0x04, 0x00, 0x00, 0x00},
...
void AES::MixColumns(uint8_t *block) const {
    for (uint32_t i = 0; i < 4; i++) {
        ...
        block[i * 4 + 0] = gf8_lookup[0x02][a ^ b] ^ b ^ c ^ d;
        block[i * 4 + 1] = gf8_lookup[0x02][b ^ c] ^ a ^ c ^ d;
        block[i * 4 + 2] = gf8_lookup[0x02][c ^ d] ^ a ^ b ^ d;
        block[i * 4 + 3] = gf8_lookup[0x02][d ^ a] ^ a ^ b ^ c;
    }
}
...
void AES::InvMixColumns(uint8_t *block) const {
    for (uint32_t i = 0; i < 4; i++) {
        ...
        uint8_t c = block[i * 4 + 2];
        uint8_t d = block[i * 4 + 3];
        block[i * 4 + 0] = gf8_lookup[0x0e][a] ^ gf8_lookup[0x0b][b] ^ gf8_lookup[0x0d][c] ^ gf8_lookup[0x09][d];
        block[i * 4 + 1] = gf8_lookup[0x09][a] ^ gf8_lookup[0x0e][b] ^ gf8_lookup[0x0b][c] ^ gf8_lookup[0x0d][d];
        block[i * 4 + 2] = gf8_lookup[0x0d][a] ^ gf8_lookup[0x09][b] ^ gf8_lookup[0x0e][c] ^ gf8_lookup[0x0b][d];
        block[i * 4 + 3] = gf8_lookup[0x0b][a] ^ gf8_lookup[0x0d][b] ^ gf8_lookup[0x09][c] ^ gf8_lookup[0x0e][d];
    }
}
